...
int TestDriverInvokeProcess( int argc, char* argv[] )
{
  ...
  for ( int i = 1; i < argc; ++i ) {
    ...
    std::cout << processArgv[i-1] << std::endl;
  }
...
int main(int argc, char* argv[])
{
  ...
  std::string compareFile1;
  std::string compareFile2;
  for ( int i = testArgc; i < argc; ++i ) {
    if ( strcmp( argv[i], "--compare" ) == 0 && i + 2 < argc ) {
      compareFile1 = std::string( argv[i+1] );
      compareFile2 = std::string( argv[i+2] );
      // Allow only two files to be compared at the moment
      break;
    }
  }

  std::cout << "Comparing " << compareFile1 << " and " << compareFile2 << std::endl;

  // Launch the test command as a subprocess
  if ( TestDriverInvokeProcess( testArgc, argv ) != EXIT_SUCCESS ) {
    std::cout << "Failed to invoke test process " << argv[1] << std::endl;
    return EXIT_FAILURE;
  }
